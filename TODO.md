...
- [x] **Initialize workspace.**
  - Created `Cargo.toml` workspace manifest with the five member crates.
  - Pinned Rust toolchain via `rust-toolchain.toml` (latest stable at init).
  - Committed `.gitignore`, `.editorconfig`.
- [x] **Commit license files verbatim.**
  - Fetched CC BY 4.0 legal code from `creativecommons.org` and committed as `LICENSE-SPEC`.
  - Fetched Apache-2.0 legal code from `apache.org` and committed as `LICENSE-APACHE`.
  - Committed `LICENSE-MIT` from the text in Stage 1.
  - Committed `NOTICE` from Stage 1.
- [x] **Commit Stage 1–4 documents.**
  - `README.md`, `SPEC.md`, `ARCHITECTURE.md`, `TODO.md`.
  - All cross-references verified (README → SPEC sections, SPEC → ARCHITECTURE concepts).

### `agi4-core` crate — the verdict logic

- [x] **Define core enums.**
  - `Conjunct`, `ConjunctStatus`, `Verdict`, `NotAttestedReason`, `MissingEvidence`, `ConsistencyRule`.
  - All derived: `Debug`, `Clone`, `PartialEq`, `Eq`, `Serialize`, `Deserialize`.
- [x] **Define `Evidence` and bounded value types.**
...
- [x] **SemVer baseline.**
  - Added `cargo-semver-checks` to CI for `agi4-core` only.
  - Tagged baseline at v0.1.0.

### `agi4-schema` crate — output JSON schema

- [x] **Define output types matching ARCHITECTURE §7.**
  - `VerdictOutput`, `ConjunctReport`, `EvidenceReport`, `ConsistencyReport`, `ModelMeta`.
- [x] **Round-trip serialization tests.**
  - Constructed representative `VerdictOutput`, serialized, deserialized, asserted equality.
- [x] **JSON schema export.**
  - Generated JSON Schema (Draft 2020-12) from the Rust types using `schemars`.
  - Committed schema to `/schema/agi4-output-v0.1.0.json`.
  - CI step regenerates schema and fails if drifted from committed file.
- [x] **Known-gaps embedding.**
...
- [x] **Snapshot test.**
  - Rendered fixture `VerdictOutput`, asserted Markdown matches committed golden file.

### `agi4` facade crate — library + binary

- [x] **Create `agi4` crate with `[lib]` and `[[bin]]` targets.**
  - Version: `0.1.0` (matches spec).
- [x] **Curated public API in `lib.rs`.**
  - Re-exported from `agi4-core`: `Conjunct`, `ConjunctStatus`, `Verdict`, `Evidence`, `SourceValue`, `Provenance`, threshold constants module.
  - Re-exported from `agi4-schema`: `VerdictOutput` and child types.
  - Did not re-export `agi4-adapters` internals or `agi4-report` internals.
- [x] **CLI binary in `main.rs`.**
...
- [x] **Add `.github/workflows/ci.yml` (or equivalent) per ARCHITECTURE §9.**
  - `cargo fmt --check`
...
- [x] **Pin clippy lints in `clippy.toml`.**
  - Denied `unwrap_used`, `expect_used` (outside tests).
- [x] **Schema-drift check.**
  - CI step regenerates `/schema/agi4-output-v0.1.0.json` and fails if it differs from committed.
- [x] **Spec-to-code grep check.**
  - Script asserts every constant in `threshold.rs` is referenced in at least one evaluation function.
- [x] **Reproducibility.**
...
- [x] `cargo install --path crates/agi4` succeeds locally.
- [x] `agi4 attest --model example --fixture tests/fixtures/example/` produces a valid JSON verdict.
- [x] `agi4 render --input <that JSON>` produces a readable Markdown report.
- [x] Tagged `v0.1.0` on the `agi4` crate. Other crates tagged at their own current versions.
- [x] Published to crates.io: `agi4-core`, `agi4-schema`, `agi4-adapters`, `agi4-report`, `agi4`.
...
- [x] **Adapter: ARC Prize (ARC-AGI-2, ARC-AGI-3).**
...
- [x] **Adapter: HLE.**
  - Implemented with fixture and round-trip test.
- [x] **Adapter: GPQA Diamond.**
  - Via Epoch AI benchmark hub if direct source unavailable.
  - Frozen fixture and round-trip test.
- [x] **Adapter: GDPval / GDPval-AA.**
  - Artificial Analysis's third-party implementation for verifiability.
  - Fixture and round-trip test.
- [x] **Adapter: RLI (Remote Labor Index).**
  - Fixture and round-trip test.
- [x] **Adapter: APEX-Agents.**
  - Fixture and round-trip test.
- [x] **Adapter: OSWorld.**
  - Fixture and round-trip test.
- [x] **Adapter: RE-Bench.**
  - Fixture and round-trip test.
- [x] **Adapter: SWE-bench Verified (pass@5).**
  - Rejects pass@1-only data and returns `insufficient_data` for that source constraint.
  - Fixture and round-trip test.

### Live attestation

- [x] **Wire `agi4 attest --live` to actually fetch.**
...
- [x] **Multiple live verdicts.**
  - Five frontier models with sufficient public upstream data.
  - Live attestations ran and committed.
  - JSON + Markdown reports in `/attestations/v0.1.0/` and `/attestations/v0.1.1/`.
- [x] **README update.**
  - Added "First attestations" section linking to the committed verdicts.

### v0.1.1 release gate

- [x] All adapters implemented with fixtures and round-trip tests.
- [x] Five live attestations committed for current frontier models.
- [x] No spec changes from v0.1.0.
- [x] `agi4` facade crate stays at `0.1.0` (spec unchanged). Individual library crates bumped per their own changes.
