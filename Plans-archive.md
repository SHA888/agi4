...
Completed phases (Phase 1–3) moved out of `Plans.md` to keep it under the 200-line soft limit. Full task-level detail with commit hashes preserved below.

<details>
<summary><strong>Phase 1: v0.1.0 — Scaffold and First Verdict</strong> (2026-05-26, all 19 tasks cc:done)</summary>
...
<details>
<summary><strong>Phase 2: v0.1.1 — Real Adapters, First Live Attestation</strong> (2026-06-02, all tasks cc:done)</summary>
...
result the first attestation (Task 2.12) was **invalid** — it reported `attested`
for Claude 3.5 Sonnet from synthetic data that would be `not_attested` under the
real SPEC thresholds. Tasks 2.12–2.13 outputs were superseded by the fixes
...
| 2.21 | M | `main.rs:58` `.unwrap()` and `Default` impls' `.expect()` (`CachingFetcher` + all 9 adapters) panic in non-test code, violating the no-unwrap/no-panic rule | Replace with `?` / fallible construction; enable `clippy::unwrap_used` + `expect_used` deny in CI to enforce | 2.11 | cc:done [51083cd] |
...
| 2.26 | L | Seven single-value adapters are ~300-line near-duplicates with per-adapter error enums + test modules (~4000 LOC copy-paste); crate-level `AdapterError` is unused | Extract a generic `FractionSource` helper + parameterized tests; consume shared `AdapterError` | 2.1-2.9 | cc:done [5cbc73f] All 7 adapters refactored; ~420 LOC eliminated |

##### Task 2.26 — Refactoring Pattern (Complete 4 Remaining Adapters)

**Canonical pattern** established by HLE, gdpval, rli. Repeated for: `gpqa_diamond`, `apex_agents`, `osworld`, `re_bench`.
...
**Verification**: `cargo test -p agi4-adapters --lib` passed all 175+ tests.
...
**Next:** Phase 3 (v0.1.2 → Calibration) — start calibration work with first live verdicts from frontier models (Task 3.1).

</details>

<details>
<summary><strong>Phase 3: v0.1.2 — Calibration Based on First Attestation</strong> (2026-06-29, all tasks cc:done)</summary>
...
| 3.3 | Update SPEC.md thresholds and corresponding code constants | Spec and `threshold.rs` updated in sync, verdict-impact analysis included | 3.2 | cc:done [688b908] |
| 3.4 | Re-attest same models under v0.1.2, commit new verdicts | Five verdicts committed alongside v0.1.0 verdicts | 3.3 | cc:done [376e82c] |
| 3.5 | Document threshold movements in CHANGELOG.md | CHANGELOG records rationale for each threshold change | 3.4 | cc:done [3f7f9b7] |
